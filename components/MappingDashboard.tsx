...
import { Database, ArrowRight, Save, RefreshCw, Trash2, Plus } from 'lucide-react';
...
const MappingDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    if (Object.keys(s).length > 0 && !activeTab) {
      setActiveTab(Object.keys(s)[0]);
    }
    setLoading(false);
  };
  ...
  const handleUpdate = (m: FieldMapping, field: keyof FieldMapping, val: string) => {
    ...
    saveFieldMapping(updated); // Save immediately (or debounced in real app)
